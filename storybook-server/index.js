...
import path, { posix, isAbsolute, resolve, dirname } from "node:path";
import "esbuild";
...
import { encodeBase64, decodeHex, decodeBase64, encodeHexUpperCase } from "@oslojs/encoding";
...
function collapseDuplicateLeadingSlashes(path2) {
  if (!path2) {
    return path2;
  }
  return path2.replace(MANY_LEADING_SLASHES, "/");
}
const MANY_SLASHES = /\/{2,}/g;
function collapseDuplicateSlashes(path2) {
  if (!path2) {
    return path2;
  }
  return path2.replace(MANY_SLASHES, "/");
}
...
function joinPaths(...paths) {
  return paths.filter(isString).map((path2, i) => {
    if (i === 0) {
...
const errorMap = (issue) => {
  ...
  if (issue.code === "invalid_union") {
    ...
    const details = [...typeOrLiteralErrByPath.entries()].filter(([, error2]) => error2.expected.length === issue.errors.flat().length).map(
      ([key, error2]) => key === baseErrorPath ? (
        // Avoid printing the key again if it's a base error
        `> ${getTypeOrLiteralMsg(error2)}`
      ) : `> ${prefix(key, getTypeOrLiteralMsg(error2))}`
    );
    if (details.length === 0) {
      ...
      if (expectedShapes.length) {
        ...
        details.push("> Received `" + stringify$1(issue.input) + "`");
      }
    }
    return {
...
const getTypeOrLiteralMsg = (error2) => {
  if (typeof error2.received === "undefined" || error2.received === "undefined")
    return error2.message ?? "Required";
  const expectedDeduped = new Set(error2.expected);
  switch (error2.code) {
    case "invalid_type":
      return `Expected type \`${unionExpectedVals(expectedDeduped)}\`, received \`${stringify$1(
        error2.received
      )}\``;
    case "invalid_literal":
      return `Expected \`${unionExpectedVals(expectedDeduped)}\`, received \`${stringify$1(
        error2.received
      )}\``;
  }
};
const prefix = (key, msg) => key.length ? `**${key}**: ${msg}` : msg;
const unionExpectedVals = (expectedVals) => [...expectedVals].map((expectedVal) => stringify$1(expectedVal)).join(" | ");
const flattenErrorPath = (errorPath) => errorPath.join(".");
const stringify$1 = (val) => JSON.stringify(val, null, 1).split(newlinePlusWhitespace).join(" ");
...
function getEventPrefix({ level, label }) {
  ...
  if (level === "error" || level === "warn") {
    prefix2.push(colors.bold(timestamp));
    prefix2.push(`[${level.toUpperCase()}]`);
  } else {
    prefix2.push(timestamp);
  }
  if (label) {
    prefix2.push(`[${label}]`);
  }
  if (level === "error") {
    return colors.red(prefix2.join(" "));
  }
  if (level === "warn") {
    return colors.yellow(prefix2.join(" "));
  }
  if (prefix2.length === 1) {
    return colors.dim(prefix2[0]);
  }
  return colors.dim(prefix2[0]) + " " + colors.blue(prefix2.splice(1).join(" "));
}
class Logger {
  options;
  constructor(options) {
...
function getParams(route, pathname) {
  ...
  const path2 = pathname.endsWith(".html") && !routeHasHtmlExtension(route) ? pathname.slice(0, -5) : pathname;
  const allPatterns = [route, ...route.fallbackRoutes].map((r) => r.pattern);
  const paramsMatch = allPatterns.map((pattern) => pattern.exec(path2)).find((x) => x);
...
function compileCacheRoutes(routes, base, trailingSlash) {
  const compiled = Object.entries(routes).map(([path2, options]) => {
    const segments = removeLeadingForwardSlash(path2).split("/").filter(Boolean).map((s) => getParts(s, path2));
    const pattern = getPattern(segments, base, trailingSlash);
    return { pattern, options, segments, route: path2 };
  });
  compiled.sort(
    (a, b) => routeComparator(
      { segments: a.segments, route: a.route },
      { segments: b.segments, route: b.route }
    )
  );
  return compiled;
}
...
class RenderContext {
  ...
  async createResult(mod, ctx) {
    ...
    const { clientDirectives, inlinedScripts, compressHTML, manifest, renderers, resolve: resolve2 } = pipeline;
...
function splitAssetPath(path2) {
  const parsed = new URL(path2, URL_PARSE_BASE);
  const isAbsolute2 = URL.canParse(path2);
  const pathname = !isAbsolute2 && !path2.startsWith("/") ? parsed.pathname.slice(1) : parsed.pathname;
  return {
    pathname,
...
class ContainerPipeline extends Pipeline {
  ...
  static create({
    ...
    return new ContainerPipeline(logger, manifest, "development", renderers, resolve2, streaming);
  }
...
class experimental_AstroContainer {
  ...
  static async create(containerOptions = {}) {
    const { streaming = false, manifest, renderers = [], resolve: resolve2 } = containerOptions;
  ...
  #insertRoute({
    ...
    const pathUrl = new URL(path2, "https://example.com");
...
const DEFAULT_SANITIZE_HTML_OPTIONS = {
  allowedTags: [
    "a",
    "abbr",
    "b",
...
function mergeSanitizeHtmlOptions(userOptions) {
  ...
  if (isRecord$2(DEFAULT_SANITIZE_HTML_OPTIONS.allowedAttributes) && isRecord$2(userOptions?.allowedAttributes)) {
    merged.allowedAttributes = {
      ...DEFAULT_SANITIZE_HTML_OPTIONS.allowedAttributes,
      ...userOptions.allowedAttributes
    };
  }
  if (isRecord$2(userOptions?.allowedClasses)) {
    merged.allowedClasses = {
      ...isRecord$2(DEFAULT_SANITIZE_HTML_OPTIONS.allowedClasses) ? DEFAULT_SANITIZE_HTML_OPTIONS.allowedClasses : {},
      ...userOptions.allowedClasses
    };
  }
  if (isRecord$2(userOptions?.allowedStyles)) {
    merged.allowedStyles = {
      ...isRecord$2(DEFAULT_SANITIZE_HTML_OPTIONS.allowedStyles) ? DEFAULT_SANITIZE_HTML_OPTIONS.allowedStyles : {},
      ...userOptions.allowedStyles
    };
  }
  return merged;
}
function normalizePathList(value, path2) {
  if (value === void 0) {
    return [];
  }
  if (!Array.isArray(value)) {
    throw new Error(`${path2} must be an array of dot-path patterns.`);
  }
  const unique = /* @__PURE__ */ new Set();
  value.forEach((entry, index) => {
    if (typeof entry !== "string") {
      throw new Error(`${path2}[${index}] must be a string.`);
    }
    const normalized = entry.trim();
    if (!normalized) {
      throw new Error(`${path2}[${index}] cannot be an empty string.`);
    }
...
function shouldSanitizePath(path2, patterns) {
  return patterns.some((pattern) => matchesPathPattern(path2, pattern));
}
function matchesPathPattern(path2, pattern) {
...
const moduleMockStorage = new AsyncLocalStorage();
...
const staticClientModules = {
  "astro:scripts/page.js": "./_astro/astro_3Ascripts_2Fpage-Higt3U9E.js",
  "astro:scripts/before-hydration.js": "./_astro/astro_3Ascripts_2Fbefore-hydration-Higt3U9E.js"
};
const clientModulesResolvers = [];
function resolveClientModules(specifier) {
...
async function handlerFactory(_integrations, options) {
  ...
  let renderQueue2 = Promise.resolve(void 0);
  ...
  return async function handler(data) {
    ...
    const resultPromise = renderQueue2.then(executeRender, executeRender);
    renderQueue2 = resultPromise.then(
      () => void 0,
      () => void 0
    );
...
const storybookAstroStoryRulesConfigFilePath = void 0;
const storybookAstroServerAuthHeader = "authorization";
const app = new Hono();
...
async function createRenderHandler() {
  return handlerFactory([], {
    ...
    resolveRulesConfigModule: () => storybookAstroStoryRulesConfig,
    loadModule: async (componentId) => {
      const component = astroFiles[componentId];
      if (!component) {
        throw new Error(
...
function isRequestAuthorized(headerValue) {
  {
    return true;
  }
}
